...
import { UserProfile, CharismaEntry } from '@/constants/theme';
...
export const updateProfile = async (userId: string, updates: Partial<UserProfile>) => {
  const { data, error } = await supabase
    ...
    .update({
      ...
      whatsapp: updates.socialLinks?.whatsapp,
      tiktok: updates.socialLinks?.tiktok,
      total_entries: updates.totalEntries,
      streak: updates.streak,
      top_charisma: updates.topCharisma,
    })
    .eq('id', userId)
    .select()
    .single();
